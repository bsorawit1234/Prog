...
import { calculateTrainingStats, type StatPoint } from '../lib/training/stats';
...
type TrackingType = 'weight_reps' | 'bodyweight_reps' | 'assisted' | 'duration' | 'distance';
type LibraryExercise = { id: string; name: string; primary_muscle: string; equipment: string; tracking_type?: TrackingType; default_weight_unit?: Unit; archived_at?: string | null };
...
type TrainingStats = { sessions: number; volumeKg: number; completedSets: number; durationSeconds: number; personalRecords: number; currentWeekVolumeKg: number; previousWeekVolumeKg: number; weeklyPoints: StatPoint[]; volumeByDate: Record<string, number>; exerciseMaxes: Record<string, number> };
type BodyMeasurement = { id: string; measured_at: string; body_weight: number | null; body_weight_unit: Unit | null; body_weight_kg: number | null; body_fat_percent: number | null; notes: string | null };
...
function emptyStats(): TrainingStats { return { sessions: 0, volumeKg: 0, completedSets: 0, durationSeconds: 0, personalRecords: 0, currentWeekVolumeKg: 0, previousWeekVolumeKg: 0, weeklyPoints: [], volumeByDate: {}, exerciseMaxes: {} }; }
...
export default function Home() {
  ...
  const [measurementUnit, setMeasurementUnit] = useState<Unit>('kg');
  const [measurementBodyFat, setMeasurementBodyFat] = useState('');
  const [measurementNotes, setMeasurementNotes] = useState('');
  const [measurementMessage, setMeasurementMessage] = useState('');
  const [syncStatus, setSyncStatus] = useState<'local' | 'syncing' | 'synced' | 'needs_attention'>('local');
  ...
  useEffect(() => {
    ...
    if (params.get('tab') === 'progress') window.setTimeout(() => setTab('progress'), 0);
    if (params.get('start') === 'workout') window.setTimeout(() => setShowStart(true), 0);
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    const updateConnectivity = () => setSyncStatus(navigator.onLine ? (user ? 'synced' : 'local') : 'needs_attention');
    window.setTimeout(updateConnectivity, 0);
  ...
  useEffect(() => {
    ...
    async function loadTrainingData() {
      ...
      const [{ data: sessions }, { data: library }, { data: programs }, { data: routines }, { data: routineExercises }, { data: measurements }] = await Promise.all([
        ...
        supabase.from('exercises').select('id,name,primary_muscle,equipment,tracking_type,default_weight_unit,archived_at').eq('owner_id', user.id).is('archived_at', null).order('name').limit(200),
        ...
        supabase.from('routine_exercises').select('id,routine_id,exercise_id,position,rest_seconds,exercises(id,name,primary_muscle,equipment,tracking_type,default_weight_unit)').order('position'),
        supabase.from('body_measurements').select('id,measured_at,body_weight,body_weight_unit,body_weight_kg,body_fat_percent,notes').eq('owner_id', user.id).order('measured_at', { ascending: false }).limit(30),
      ]);
      if (cancelled) return;
      const nextSessions = (sessions ?? []) as RecentSession[];
      setRecentSessions(nextSessions);
      setExerciseLibrary((library ?? []) as LibraryExercise[]);
      setBodyMeasurements((measurements ?? []) as BodyMeasurement[]);
      ...
      if (!sessionIds.length) { setPreviousByExercise({}); setTrainingStats(emptyStats()); setSyncStatus('synced'); return; }
      ...
      const statSets: { sessionId: string; startedAt: string; exerciseKey: string; weightKg: number; reps: number; completed: boolean }[] = [];
      ...
      orderedExerciseRows.forEach((row) => {
        ...
        (Array.isArray(row.workout_sets) ? row.workout_sets : []).forEach((set) => {
          statSets.push({ sessionId: row.session_id, startedAt: nextSessions.find((session) => session.id === row.session_id)?.started_at || new Date().toISOString(), exerciseKey: key, weightKg: Number(set.normalized_weight_kg || 0), reps: Number(set.reps || 0), completed: Boolean(set.completed_at) });
        });
      });
      setPreviousByExercise(previous);
      setTrainingStats(calculateTrainingStats(nextSessions, statSets));
  ...
  async function editLibraryExercise(exercise: LibraryExercise) {
    if (!supabase || !user) { setExerciseMessage('Sign in to edit your exercise library.'); return; }
    const name = window.prompt('Exercise name', exercise.name)?.trim();
    if (!name || name === exercise.name) return;
    const { data, error } = await supabase.from('exercises').update({ name }).eq('id', exercise.id).eq('owner_id', user.id).select('id,name,primary_muscle,equipment,tracking_type,default_weight_unit').single();
    if (error || !data) { setExerciseMessage(error?.message || 'Could not update this exercise.'); return; }
    setExerciseLibrary((current) => current.map((item) => item.id === exercise.id ? data as LibraryExercise : item).sort((a, b) => a.name.localeCompare(b.name)));
  }
  async function archiveLibraryExercise(exercise: LibraryExercise) {
    if (!supabase || !user || !window.confirm(`Archive ${exercise.name}? It will stay in workout history but stop appearing in the library.`)) return;
    const { error } = await supabase.from('exercises').update({ archived_at: new Date().toISOString() }).eq('id', exercise.id).eq('owner_id', user.id);
    if (error) { setExerciseMessage(error.message); return; }
    setExerciseLibrary((current) => current.filter((item) => item.id !== exercise.id));
  }
  async function saveMeasurement() {
    const weight = Number(measurementWeight);
    if (!supabase || !user) { setMeasurementMessage('Sign in to save measurements to Supabase.'); return; }
    if (!Number.isFinite(weight) || weight <= 0) { setMeasurementMessage('Enter a body weight first.'); return; }
    const bodyWeightKg = measurementUnit === 'lb' ? weight / 2.20462 : weight;
    const { data, error } = await supabase.from('body_measurements').insert({ owner_id: user.id, body_weight: weight, body_weight_unit: measurementUnit, body_weight_kg: bodyWeightKg, body_fat_percent: measurementBodyFat ? Number(measurementBodyFat) : null, notes: measurementNotes.trim() || null }).select('id,measured_at,body_weight,body_weight_unit,body_weight_kg,body_fat_percent,notes').single();
    if (error || !data) { setMeasurementMessage(error?.message || 'Could not save this measurement.'); return; }
    setBodyMeasurements((current) => [data as BodyMeasurement, ...current].slice(0, 30));
    setMeasurementWeight(''); setMeasurementBodyFat(''); setMeasurementNotes(''); setMeasurementMessage('Saved.');
  }
  function exportHistory() {
    if (!recentSessions.length) { setSaveError('There is no saved history to export yet.'); return; }
    const rows = [['date', 'workout', 'duration_seconds'], ...recentSessions.map((session) => [new Date(session.started_at).toISOString(), session.name, String(session.duration_seconds || 0)])];
    const csv = rows.map((row) => row.map((value) => `"${value.replaceAll('"', '""')}"`).join(',')).join('\n');
    const link = document.createElement('a');
    link.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' }));
    link.download = `prog-history-${dateKey(new Date())}.csv`;
    link.click();
  ...
  return <main className="app-shell">
    ...
    </nav><Kicker>LIBRARY</Kicker><nav className="side-nav library-nav"><button className="nav-item" onClick={() => { setExerciseMessage(''); setShowExercisePicker(true); }}><Icon>＋</Icon>Exercises</button><button className="nav-item" onClick={() => { setMeasurementMessage(''); setShowMeasurements(true); }}><Icon>⚙</Icon>Measurements</button></nav><div className="profile"><div className="avatar">BS</div><div><strong>Boa Sorawit</strong><span>Personal space</span></div><button aria-label="More profile actions">•••</button></div></aside>
    <section className="main-column"><header className="topbar"><div className="mobile-logo"><Logo /></div><div className="top-date"><Kicker>{todayLabel}</Kicker><span className="saved-state"><i /> {user ? `Supabase ${syncStatus}` : 'Preview mode'}</span></div><div className="top-actions"><button className="theme-toggle" aria-label={`Switch to ${theme === 'light' ? 'dark' : 'light'} theme`} aria-pressed={theme === 'dark'} onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')}>{theme === 'light' ? '☾' : '☀'}</button>{user ? <button className="user-pill" onClick={signOut}>{user.email?.split('@')[0]} · sign out</button> : <button className="sign-in-button" onClick={() => { setAuthMessage(''); setAuthOpen(true); }}>Sign in</button>}<div className="avatar small">BS</div></div></header>
      ...
      {tab === 'calendar' && <Calendar recentSessions={recentSessions} stats={trainingStats} onOpenSession={openHistory} onOpen={() => startWorkout(activeRoutine)} />}
      {tab === 'progress' && <Progress stats={trainingStats} measurements={bodyMeasurements} onMeasurements={() => { setMeasurementMessage(''); setShowMeasurements(true); }} onExport={exportHistory} />}
    </section>
    ...
    {showExercisePicker && <ExercisePickerModal library={exerciseLibrary} search={exerciseSearch} name={exerciseName} muscle={exerciseMuscle} message={exerciseMessage} onSearch={setExerciseSearch} onName={setExerciseName} onMuscle={setExerciseMuscle} onPick={addExerciseToWorkout} onCreate={createExercise} onEdit={editLibraryExercise} onArchive={archiveLibraryExercise} close={() => setShowExercisePicker(false)} />}
    {showMeasurements && <MeasurementModal measurements={bodyMeasurements} weight={measurementWeight} unit={measurementUnit} bodyFat={measurementBodyFat} notes={measurementNotes} message={measurementMessage} onWeight={setMeasurementWeight} onUnit={setMeasurementUnit} onBodyFat={setMeasurementBodyFat} onNotes={setMeasurementNotes} onSave={saveMeasurement} close={() => setShowMeasurements(false)} />}
...
function Calendar({ recentSessions, stats, onOpenSession, onOpen }: { recentSessions: RecentSession[]; stats: TrainingStats; onOpenSession: (sessionId: string) => void; onOpen: () => void }) {
  ...
  return <div className="page"><div className="page-heading"><div><Kicker>TRAINING HISTORY</Kicker><h1>Your calendar</h1><p>Every session counts. Keep building the picture.</p></div><button className="secondary-button" onClick={() => setMonth(new Date())}>Today</button></div><div className="calendar-layout"><article className="calendar-card"><div className="calendar-title"><button aria-label="Previous month" onClick={() => setMonth(new Date(year, monthIndex - 1, 1))}>‹</button><h2>{month.toLocaleString('en-US', { month: 'long', year: 'numeric' })}</h2><button aria-label="Next month" onClick={() => setMonth(new Date(year, monthIndex + 1, 1))}>›</button></div><div className="weekday-row">{weekdayLabels.map((label) => <span key={label}>{label}</span>)}</div><div className="calendar-grid">{Array.from({ length: firstDay }, (_, i) => <span className="empty-day" key={`empty-${i}`} />)}{Array.from({ length: daysInMonth }, (_, i) => { const day = i + 1; const sessions = sessionsByDay.get(dateKey(new Date(year, monthIndex, day))) || []; return <button className={sessions.length ? 'calendar-day trained' : 'calendar-day'} key={day} onClick={() => sessions[0] && onOpenSession(sessions[0].id)}><span>{day}</span>{sessions.length > 0 && <i />}{sessions.length > 1 && <em>{sessions.length}</em>}</button>; })}</div><div className="legend"><span><i /> Workout</span><span><i /> Rest day</span></div></article><aside className="month-summary"><Kicker>{month.toLocaleString('en-US', { month: 'long' }).toUpperCase()} AT A GLANCE</Kicker><div className="month-stat">{monthSessions.length}<small>workouts</small></div>{[['Training days', `${new Set(monthSessions.map((session) => dateKey(session.started_at))).size}`], ['Total volume', monthSessions.length ? `${Math.round(monthSessions.reduce((sum, session) => sum + (stats.volumeByDate[dateKey(session.started_at)] || 0), 0)).toLocaleString()} kg` : '—'], ['Time trained', formatDuration(monthSessions.reduce((sum, session) => sum + (session.duration_seconds || 0), 0))], ['Personal records', stats.sessions ? `${stats.personalRecords} overall` : '—']].map(([label, value]) => <div className="summary-row" key={label}><span>{label}</span><b>{value}</b></div>)}<Kicker>HOW TO READ IT</Kicker><p className="month-help">Tap a trained day to open the saved session and review every set.</p></aside></div><div className="section-heading calendar-session-heading"><div><Kicker>{monthSessions.length ? 'LATEST SAVED SESSION' : 'NO SESSIONS THIS MONTH'}</Kicker><h2>{monthSessions[0]?.name || 'Ready when you are'}</h2></div><button className="text-button" onClick={onOpen}>Start workout →</button></div>{monthSessions[0] ? <button className="session-card" onClick={() => onOpenSession(monthSessions[0].id)}><span className="code coral">{monthSessions[0].name.slice(0, 2).toUpperCase()}</span><div className="activity-name"><strong>{monthSessions[0].name}</strong><span>{formatDuration(monthSessions[0].duration_seconds)} · saved session</span></div><div className="session-stats"><b>Saved</b><span>Open details</span></div><span className="arrow">→</span></button> : <div className="empty-option">Complete your first workout to see it here.</div>}</div>;
}

function Progress({ stats, measurements, onMeasurements, onExport }: { stats: TrainingStats; measurements: BodyMeasurement[]; onMeasurements: () => void; onExport: () => void }) {
  const [range, setRange] = useState(12);
  const points = stats.weeklyPoints.slice(-range);
  const maxVolume = Math.max(1, ...points.map((point) => point.volumeKg));
  const latest = measurements[0];
  const prior = measurements[1];
  const weightChange = latest?.body_weight_kg && prior?.body_weight_kg ? latest.body_weight_kg - prior.body_weight_kg : null;
  const maxBodyWeight = Math.max(1, ...measurements.map((item) => item.body_weight_kg || 0));
  return <div className="page"><div className="page-heading"><div><Kicker>THE BIG PICTURE</Kicker><h1>Progress, measured.</h1><p>Strength is a trend, not a single number.</p></div><div className="heading-actions"><button className="secondary-button" onClick={onExport}>Export CSV</button><select className="secondary-button range-select" value={range} onChange={(event) => setRange(Number(event.target.value))} aria-label="Chart range"><option value="4">Last 4 weeks</option><option value="8">Last 8 weeks</option><option value="12">Last 12 weeks</option></select></div></div><div className="progress-grid"><article className="progress-card"><div className="stat-label"><span className="stat-icon">↗</span>TRAINING VOLUME<span className="chart-range">12 weeks</span></div><div className="progress-value">{Math.round(stats.volumeKg).toLocaleString()} <small>kg</small><span>{stats.sessions} sessions</span></div><div className="real-chart">{points.length ? points.map((point) => <div className="chart-column" key={point.label}><i style={{ height: String(Math.max(8, (point.volumeKg / maxVolume) * 100)) + '%' }} title={point.label + ': ' + Math.round(point.volumeKg).toLocaleString() + ' kg'} /><small>{point.label}</small></div>) : <div className="chart-empty">Complete a workout to plot weekly volume.</div>}</div><div className="chart-labels"><span>THIS WEEK {Math.round(stats.currentWeekVolumeKg).toLocaleString()} KG</span><span>{stats.completedSets} SETS</span><span>{formatDuration(stats.durationSeconds)}</span></div></article><article className="progress-card body-card"><div className="stat-label"><span className="stat-icon violet">◒</span>BODY WEIGHT<span className="chart-range">{measurements.length} logs</span></div><div className="progress-value">{latest?.body_weight_kg ? latest.body_weight_kg.toFixed(1) : '—'} <small>kg</small><span>{weightChange === null ? 'No previous entry' : (weightChange >= 0 ? '+' : '') + weightChange.toFixed(1) + ' kg'}</span></div><div className="body-change">{latest ? 'Logged ' + new Date(latest.measured_at).toLocaleDateString() : 'Track body weight alongside training'} <span>{latest?.body_fat_percent ? latest.body_fat_percent + '% body fat' : 'Optional body fat'}</span></div><div className="weight-bars">{measurements.slice(0, 10).reverse().map((measurement, index) => <i style={{ height: String(Math.max(12, Math.min(100, ((measurement.body_weight_kg || 0) / maxBodyWeight) * 100))) + '%' }} key={measurement.id || index} />)}</div><button className="card-action" onClick={onMeasurements}>Log body measurement ＋</button></article></div><div className="section-heading"><div><Kicker>PERFORMANCE</Kicker><h2>Personal records</h2></div><span className="text-button">Calculated from completed sets</span></div><div className="pr-table"><div className="pr-head"><span>METRIC</span><span>VALUE</span><span>DETAIL</span><span>STATUS</span></div><div className="pr-row"><strong>Completed sessions</strong><span>{stats.sessions}</span><span>{formatDuration(stats.durationSeconds)}</span><span>Saved</span></div><div className="pr-row"><strong>Completed sets</strong><span>{stats.completedSets}</span><span>{Math.round(stats.volumeKg).toLocaleString()} kg volume</span><span>Tracked</span></div><div className="pr-row"><strong>Personal records</strong><span>{stats.personalRecords}</span><span>Highest completed load per exercise</span><span>{stats.sessions ? 'Calculated' : 'Waiting'}</span></div></div></div>;
}
...
function MeasurementModal({ measurements, weight, unit, bodyFat, notes, message, onWeight, onUnit, onBodyFat, onNotes, onSave, close }: { measurements: BodyMeasurement[]; weight: string; unit: Unit; bodyFat: string; notes: string; message: string; onWeight: (value: string) => void; onUnit: (value: Unit) => void; onBodyFat: (value: string) => void; onNotes: (value: string) => void; onSave: () => void; close: () => void }) {
  return <Modal title="Body measurements" kicker="PROGRESS CHECK-IN" close={close}><p className="picker-copy">Keep bodyweight and optional body-fat entries alongside your training history.</p><div className="measurement-form"><label>Body weight<input type="number" min="0" step="0.1" value={weight} onChange={(event) => onWeight(event.target.value)} placeholder="e.g. 78.5" /></label><label>Unit<select value={unit} onChange={(event) => onUnit(event.target.value as Unit)}><option value="kg">kg</option><option value="lb">lb</option></select></label><label>Body fat %<input type="number" min="0" max="100" step="0.1" value={bodyFat} onChange={(event) => onBodyFat(event.target.value)} placeholder="Optional" /></label><label className="measurement-notes">Notes<textarea value={notes} onChange={(event) => onNotes(event.target.value)} placeholder="How are you feeling?" rows={3} /></label></div><button className="primary-button auth-submit" onClick={onSave}>Save measurement</button>{message && <p className="auth-message" role="status">{message}</p>}<div className="measurement-history"><Kicker>RECENT ENTRIES</Kicker>{measurements.slice(0, 5).map((measurement) => <div className="measurement-row" key={measurement.id}><span>{new Date(measurement.measured_at).toLocaleDateString()}</span><strong>{measurement.body_weight || '—'} {measurement.body_weight_unit || 'kg'}</strong><small>{measurement.body_fat_percent ? `${measurement.body_fat_percent}% body fat` : ''}</small></div>)}{!measurements.length && <p className="empty-option">No measurements yet.</p>}</div></Modal>;
}
...
function ExercisePickerModal({ library, search, name, muscle, message, onSearch, onName, onMuscle, onPick, onCreate, onEdit, onArchive, close }: { library: LibraryExercise[]; search: string; name: string; muscle: string; message: string; onSearch: (value: string) => void; onName: (value: string) => void; onMuscle: (value: string) => void; onPick: (exercise: LibraryExercise) => void; onCreate: () => void; onEdit: (exercise: LibraryExercise) => void; onArchive: (exercise: LibraryExercise) => void; close: () => void }) {
  ...
  return <Modal title="Exercise library" kicker="LIBRARY" close={close}><p className="picker-copy">Search, reuse, edit or archive movements. Select a row to add it to the current workout.</p><input className="auth-input exercise-search" value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Search exercises…" aria-label="Search exercises" />{filtered.length > 0 && <div className="exercise-library-list">{filtered.map((exercise) => <div className="library-option" key={exercise.id}><button className="library-main" onClick={() => onPick(exercise)}><span className="code coral">{exercise.name.split(' ').map((word) => word[0]).slice(0, 2).join('')}</span><span><strong>{exercise.name}</strong><small>{exercise.primary_muscle} · {exercise.equipment} · {exercise.tracking_type || 'weight_reps'}</small></span><b>＋</b></button><div className="library-actions"><button onClick={() => onEdit(exercise)}>Edit</button><button onClick={() => onArchive(exercise)}>Archive</button></div></div>)}</div>}{filtered.length === 0 && <p className="empty-option">No matching exercise yet. Create it below.</p>}<div className="create-exercise-box"><Kicker>NEW MOVEMENT</Kicker><label>Name<input value={name} onChange={(event) => onName(event.target.value)} placeholder="e.g. Cable lateral raise" /></label><label>Muscle group<input value={muscle} onChange={(event) => onMuscle(event.target.value)} placeholder="e.g. Shoulders" /></label><button className="primary-button" onClick={onCreate}>Create and add to workout</button>{message && <p className="auth-message" role="status">{message}</p>}</div></Modal>;
}
